...
def run_pipeline(config_path: str | None = None, use_sample_data: bool = False) -> dict[str, Any]:
    ...
    events += satellite.ingest(cfg)
    events += aerial.ingest(cfg)
    events += ground_sensors.ingest(cfg)
    events += social_media.ingest(cfg)
    events += ingest_tracks()

    fused = fuse_events(events, cfg)

    clf = ThreatClassifier(model_dir=cfg.get("artifacts", {}).get("models_dir", "artifacts/models"))
    classified = clf.classify(fused)

    # Compute severity and persist incidents
    for inc in classified:
        inc["severity"] = severity_score(
            inc.get("threat_probability", 0.0), inc.get("features", {}), inc.get("in_geofence", False), inc.get("geofence_id")
        )

    cm = CaseManager(db_path=cfg.get("artifacts", {}).get("db_path", "artifacts/case_manager.db"))
    cm.record_incidents(classified)
    ...
    for e in events:
        ...
        for k in ("image_path",):
            if k in art:
                evidence_files.append(art[k])
    if evidence_files:
        for inc in classified:
            append_records(cfg, inc, evidence_files)
            break

    # Predictive risk map
    risk = predict_geofence_risk(cfg, classified)

    # Geofence breach summary
    breach_counts: dict[str, int] = {}
    for inc in classified:
        if inc.get("in_geofence"):
            key = inc.get("geofence_id") or "unknown"
            breach_counts[key] = breach_counts.get(key, 0) + 1

    return {
        "events": len(events),
        "fused": len(fused),
        "incidents": len(classified),
        "notified": notified,
        "risk_geofences": risk,
...
def _ensure_sample_data() -> None:
    ...
    if not social_csv.exists():
        with social_csv.open("w", encoding="utf-8", newline="") as f:
            ...
            w.writeheader()
            w.writerow({"timestamp": "2025-01-01T12:00:00+00:00", "source": "twitter", "text": "Illegal dumping spotted near river", "lat": 37.34, "lon": -122.015})
            w.writerow({"timestamp": "2025-01-01T12:05:00+00:00", "source": "news", "text": "Construction noise reported downtown", "lat": 37.345, "lon": -122.01})
            w.writerow({"timestamp": "2025-01-01T13:00:00+00:00", "source": "twitter", "text": "Great weather for a hike today", "lat": "", "lon": ""})
    ...
    if not ground_csv.exists():
        with ground_csv.open("w", encoding="utf-8", newline="") as f:
            w = csv.DictWriter(f, fieldnames=["timestamp", "lat", "lon", "pm25", "noise_db", "vibration", "temp_c"]) 
            w.writeheader()
            base = [
            ...
            for row in base:
                w.writerow({
                    "timestamp": row[0], "lat": row[1], "lon": row[2], "pm25": row[3], "noise_db": row[4], "vibration": row[5], "temp_c": row[6]
                })
